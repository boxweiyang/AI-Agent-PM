<!--
  REQ-M03：迭代需求文档单版本编辑；交互对齐 RequirementDocVersionDetailPage，API 前缀为 iterations/{id}/requirement-doc。
-->
<template>
  <div class="req-doc-detail" v-loading="loading">
    ...
    <template v-else-if="!loading && project && !ready">
      <el-card shadow="never">
        <el-result icon="folder-opened" title="尚未解锁迭代模块" :sub-title="subTitlePending">
          <template #extra>
            <el-button @click="goBackToPlanning">返回迭代列表</el-button>
          </template>
        </el-result>
      </el-card>
    </template>
    ...
    <el-card v-else-if="ready && !detail && !loading" shadow="never">
      <el-empty description="未找到该版本或已删除">
        <el-button type="primary" @click="goBackToIteration">返回迭代详情</el-button>
      </el-empty>
    </el-card>
      ...
      <template #header>
        <div class="detail-head">
          <div class="detail-head-left">
            <el-button text @click="goBackToIteration">返回迭代详情</el-button>
            <span class="detail-title">迭代需求文档 v{{ detail.version_no }}</span>
...
import type { ApiEnvelope, ProjectOneData, RequirementDocVersionDetail } from '@/types/api-contract'
import {
...
const subTitlePending = computed(
  () => `按 ${reqRef.value} 在项目中启用「迭代与 Story」后可编辑迭代需求文档。`,
)

const renderedHtml = computed(() => markdownToHtmlFragment(markdown.value || ''))

const iterationNameForAi = ref('')
...
const defaultAiPrompt = computed(() => {
  ...
  return buildRequirementDocDefaultPrompt({
    versionLabel: `迭代需求 ${iterationNameForAi.value || '（未命名迭代）'} · ${title || '（未命名版本）'}`,
...
const aiPayloadBase = computed<Record<string, unknown>>(() => {
  ...
  const iid = typeof route.params.iterationId === 'string' ? route.params.iterationId : ''
  return {
    project_id: pid,
    iteration_id: iid,
    iteration_name: iterationNameForAi.value,
...
const aiMemoryKey = computed(() => {
  ...
  if (!pid || !iid || !vid) return ''
  return `${pid}:${iid}:${vid}:iteration_requirement_doc_assist`
})
...
function goBackToPlanning() {
  ...
  if (typeof id === 'string') void router.push({ name: 'project-m03-iterations', params: { projectId: id } })
}

function goBackToIteration() {
  const id = route.params.projectId
  const iid = route.params.iterationId
  if (typeof id === 'string' && typeof iid === 'string') {
    void router.push({ name: 'project-m03-iteration-detail', params: { projectId: id, iterationId: iid } })
  }
}
...
async function fetchIterationName() {
  const pid = route.params.projectId
  const iid = route.params.iterationId
  if (typeof pid !== 'string' || typeof iid !== 'string') return
  try {
    const { data } = await apiClient.get<ApiEnvelope<{ name?: string }>>(`/api/v1/projects/${pid}/iterations/${iid}`)
    iterationNameForAi.value = data.data?.name?.trim() ?? ''
  } catch {
    iterationNameForAi.value = ''
  }
}
...
async function fetchDetail() {
  ...
  if (typeof pid !== 'string' || typeof iid !== 'string' || typeof vid !== 'string') return
  try {
    const { data } = await apiClient.get<ApiEnvelope<RequirementDocVersionDetail>>(
      `/api/v1/projects/${pid}/iterations/${iid}/requirement-doc/versions/${vid}`,
    )
    const d = data.data
    detail.value = d ?? null
...
async function doSave(mode: 'new' | 'overwrite') {
  ...
  const iid = route.params.iterationId
  const vid = route.params.versionId
  if (typeof pid !== 'string' || typeof iid !== 'string' || typeof vid !== 'string' || !detail.value?.is_latest) return
  saving.value = true
  try {
    if (mode === 'overwrite') {
...
function exportCurrent(kind: 'md' | 'html' | 'pdf') {
  ...
  if (!d) return
  const base = `迭代需求-${iterationNameForAi.value || '迭代'}-v${d.version_no}`
  const title = `迭代需求文档 v${d.version_no}`
...
watch(
  () => [route.params.projectId, route.params.iterationId, route.params.versionId],
  () => {
    void load()
  },
  { immediate: true },
