...
- **顶栏**（`AppHeaderBar`）：当前页标题（子路由 `meta.title`）、**功能** 下拉（**AI 设置**、**个人设置**、**系统设置**〔非管理员项禁用；直链 `/settings/system` 会被守卫打回首页〕、**进入最近项目**）、**深/浅色**、登录名与标签、**退出**。
...
- **侧栏**：**品牌行高度 48px**，与顶栏一致；**`PmpBrandMark` + 产品名**（全宽时）+ 当前项目名摘要 + **`el-menu`**。**三态循环**（`localStorage` 键 `pmp_project_sidebar_mode`）：① **全宽**（约 220px，文案+图标）；② **仅图标**（约 64px，`menu` `collapse`）；③ **全收起**（宽度 0，仅保留「腰钮」）。**腰钮**卡在左右区竖向中线、跨分割边框，点击在三种状态间切换；收起态用 **双右箭头**，其余态用 **双左箭头**。
